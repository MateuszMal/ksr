...
import myvectors.CharacteristicsVector;
...
public class Algorithm {
    ...
    public void categorise_single_text(CharacteristicsVector v1, int k, int[] what_to_include, int metric_switch) {
        ...
        ArrayList<StringDoubleContainer> category_scores = create_category_score_sorted_array(v1, k, what_to_include, metric_switch);
    ...
    public ArrayList<StringDoubleContainer> create_category_score_sorted_array(CharacteristicsVector v1, int k, int[] what_to_include, int metric_switch) {

        ArrayList<StringDoubleContainer> category_scores = new ArrayList<>();
        ...
        for (String s : a_learning_set.keySet()) {
            for (int i = 0; i < a_learning_set.get(s).size(); i++) {
                ...
                category_scores.add(new StringDoubleContainer(s, score));
            }
        }

        Collections.sort(category_scores, new StringDoubleContainerComparator());
    ...
    public String determine_the_category(ArrayList<StringDoubleContainer> category_scores) {
        ...
        ArrayList<StringDoubleContainer> temp_scores = new ArrayList<>();


        for (String s : determine_draw.keySet()) {
            temp_scores.add(new StringDoubleContainer(s, determine_draw.get(s)));
        }

        Collections.sort(temp_scores, new StringDoubleContainerComparator());
    ...
    public HashMap<String, ArrayList<CharacteristicsVector>> getA_test_set() {
        return a_test_set;
    }
