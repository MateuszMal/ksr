...
public class WordOccurrenceMatrix {
    ...
    public ArrayList<StringDoubleContainer> get_key_words(String category) {

        ArrayList<StringDoubleContainer> new_list_att = new ArrayList<>();
        ...
        for (int column = 0; column < words_hash_map.size(); column++) {
            if (values_array[row][column] > 0.0) {
                ...
                new_list_att.add(new StringDoubleContainer(word, values_array[row][column]));
            }
